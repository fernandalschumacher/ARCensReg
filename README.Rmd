...
`print`, `summary`, and `plot` functions also work for objects given as an output of functions `ARCensReg` and `ARtCensReg`. Function `plot` also has methods for outputs of functions `InfDiag` and `residuals`.
...
of non-normality, such as large residuals and several points outside the confidence band in the Q-Q plots.
